...
<div id="content">
 ...
 if($sqs[0][fenlei]==1){ echo "交通事故"; }else if($sqs[0][fenlei]==2){ echo "工伤"; }else if($sqs[0][fenlei]==3){ echo "人身损害"; }else if($sqs[0][fenlei]==4){ echo "人身保险"; }else{ echo "康宁保险"; } ?>'>损伤程度鉴定。
        ...
        <center><input type="botton" value="打 印" style="background:#0099FF; color:#FFFFFF;width:120px;height:30px;font-weight:bold;" onclick="window.print()"></center>
  </div>
  <div id="委托书">
		...
		<div class="table-ji">
			<table class="bg">
				...
				<tr>
					...
					<td><input type="text" name="wts[lxaddress]" value="<?php echo ($wts[0][lxaddress]); ?>" readonly="readonly"/></td>
 ...
 $i + 1; $hbs = D('huibis'); $b = 'hbly'.$i; $res = $hbs -> where("id = '$_SESSION[id]'") -> select(); if($res[0][$b] == ''){ ?>
  ...
  <div id="接谈笔录">
  	<form action="<?php echo U('Jbxx/index',array('id'=>'jietanbl'));?>" method="post">
		...
		<table class="jtblcon">
			...
			<tr>
				<td id="cons">
					...
					地点：<input type="text" name="jtbl[address]" value="" />
			...
			<tr>
				<td id="cons">
					...
					出生：
					<input type="text" name="jtbl[csyear]" value="<?php echo ($sqs[0][year]); ?>" style="width:40px;"/>年
		...
		<div id="bldiv" contenteditable="true" onkeyup="bl()">
			&nbsp;&nbsp;<?php echo ($wts[0][jazy]); ?>
		</div>
...
>>>>>>> 74a23020b3d121b4a13fd07101a1087314c8b90d
  	...
  	alert(ImagePath);
	function inser(){
	    var ImagePath = '<?php echo ($ipa[0][path]); ?>';
	    bldiv.focus();
	    document.execCommand('InsertImage','false',ImagePath);
	}
	function bl(){
	  	var ygqk=document.getElementById("ygqk");
	  	ygqk.value=bldiv.innerHTML;
    }
  </script>
