...
<div id="content">
  <div id="申请书">
	  ...
	  <table class="con" align="right">
		...
		<tr>
			<td>
				特此申请：
				<?php if(is_array($buwei)): $i = 0; $__LIST__ = $buwei;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; $sq = D('shenqing'); $res = $sq -> field('buwei') -> where("id = '$_SESSION[id]'") -> select(); if($i == $res[0][buwei]){ echo "<label>$vo[name]<input type='radio' name='sqs[buwei]' value='$vo[id]' checked></label>"; $i + 1; }else{ echo "<label>$vo[name]<input type='radio' name='sqs[buwei]' value='$vo[id]' disabled></label>"; $i + 1; } endforeach; endif; else: echo "" ;endif; ?>
			</td>
		</tr>
		<tr class="hei"></tr>
		<tr>
			<td>
				鉴定类别：
				<?php if(is_array($leibie)): $i = 0; $__LIST__ = $leibie;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; $sq = D('shenqing'); $res = $sq -> field('leibie') -> where("id = '$_SESSION[id]'") ->select(); if($i == $res[0][leibie]){ echo "<label>$vo[name](<input type='radio' name='sqs[leibie]' value='$vo[id]' checked>)</label>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"; $i + 1; }else{ echo "<label>$vo[name](<input type='radio' name='sqs[leibie]' value='$vo[id]' disabled>)</label>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"; $i + 1; } if($i == 3){ echo "<br /><br />
							&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		...
		<tr>
			<td>
				伤残鉴定分类：
				<?php if(is_array($fenlei)): $i = 0; $__LIST__ = $fenlei;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; $sq = D('shenqing'); $res = $sq -> field('fenlei') -> where("id = '$_SESSION[id]'") -> select(); if($i == $res[0][fenlei]){ echo "<label>$vo[name]<input type='radio' name='sqs[fenlei]' value='$vo[id]' checked></label>"; $i + 1; }else{ echo "<label>$vo[name]<input type='radio' name='sqs[fenlei]' value='$vo[id]' disabled></label>"; $i + 1; } endforeach; endif; else: echo "" ;endif; ?>
			</td>
		</tr>
		<tr class="hei"></tr>
		<tr>
  ...
  <div id="决定书" class="jds">
        ...
        <table>
            ...
            <tr>
                <td>
                    <input type="text" name="jds[mouth]" style="border-bottom:1px solid #000000;" value="<?php echo ($jds[0][mouth]); ?>">月<input type="text" name="jds[day]" style="border-bottom:1px solid #000000;" value="<?php echo ($jds[0][day]); ?>">日，由<?php echo session('dp'); ?>审核，根据中华人民共和国司法部令第132号《司法鉴定程
                </td>
            </tr>
            <tr>
                <td>
        ...
        <center><a href='<?php echo U("Printer/juedings",array("id"=>"$id"));?>' target="_blank"><input type="botton" value="打 印" style="background:#0099FF; color:#FFFFFF;width:120px;height:30px;font-weight:bold;cursor:pointer;"></a></center>
  </div>
  <script>
  	function vcode(){
		...
		<div class="table-ji">
			<table class="bg">
				<tr>
					...
					<td id="gdwidth4"><input type="text" name="wts[wtrname]" id="vcode1" value=""  onblur="vcode()"/></td>
				...
				<tr>
					...
					<td><input type="text" name="wts[lxphone]" id="vcode4" value="" onfocus="down()" onblur="up()"/></td>
				</tr>
				<tr>
					<td id="gdwidth">委托日期</td>
					<td><input type="text" name="wts[wtdate]" value="<?php echo date('Y-m-d');?>" /></td>
			...
			<table class="bg2">
				<tr>
					...
					<td colspan="3"><?php echo ($a[0][name]); echo ($b[0][name]); ?></td>
				</tr>
				<tr>
					<td id="gdwidth" align="center">委托鉴定要求</td>
					<td colspan="3">由<input type="text" name="wts[wtrname1]" id="vcode2"/>委托我中心要求为<?php echo ($sqs[0][name]); ?>进行<?php echo ($a[0][name]); echo ($b[0][name]); ?></td>
				</tr>
				<tr>
				...
				<tr height="80">
					...
					<td colspan="3">
						<textarea name="wts[jazy]" cols="67%" rows="3" style="outline:none;font-size:16px;border:none;"></textarea>
					</td>
				</tr>
				<tr height="200">
					<td id="gdwidth1" align="center">鉴定材料<br />目录和数量</td>
					<td colspan="3">
						<textarea name="wts[jdcl]" style="width:98%;height:98%;outline:none;"></textarea>
					</td>
				</tr>
				<tr height="200">
					<td id="gdwidth2" rowspan="2" align="center">鉴定费用<br />及<br />收取方式</td>
					<td colspan="3">
						<textarea name="wts[jdfy]" rows="5" cols="67%" style="outline:none;" onmousedown="s(event,this)"></textarea>
					</td>
				</tr>
				<tr id="feiyong">
				...
				<tr>
					...
					<td>
						<label><input type="radio" name="wts[fsfs]" value="0" />自取</label><br/><br/>
						<label><input type="radio" name="wts[fsfs]" value="1" />邮寄地址：<input type="text" name="wts[youji]"value=""/></label><br/><br/>
						<label><input type="radio" name="wts[fsfs]" value="2" />其他方式（注明）<input type="text" name="wts[qtfs]" value="" /></label>
					</td>
				</tr>
				<tr>
					<td colspan="2">
						...
						&nbsp;&nbsp;&nbsp;<label><input type="checkbox" name="wts[xysx][]" value="0" />因鉴定需要耗尽检材；</label><br/>
						&nbsp;&nbsp;&nbsp;<label><input type="checkbox" name="wts[xysx][]" value="1" />因鉴定需要可能损坏检材；</label><br />
						&nbsp;&nbsp;&nbsp;<label><input type="checkbox" name="wts[xysx][]" value="2" />鉴定完成之后无法完整退还检材；</label><br />
						&nbsp;&nbsp;&nbsp;<label><input type="checkbox" name="wts[xysx][]" value="3" />检材留样保存3个月；</label><br />
						2.&nbsp;&nbsp;&nbsp;鉴定时限：从协议签订之日起<input type="text" name="wts[jdshixian]" value="30" />个工作日完成。<br />
						&nbsp;&nbsp;&nbsp;<label><input type="checkbox" name="wts[xysx1][]" value="0" />遇复杂、疑难、特殊的技术问题，或者检验过程确需较长时间的，延长<input type="text" name="wts[ycrq]" value="30" />个工作日；</label><br />
						3.&nbsp;&nbsp;&nbsp;特殊情形鉴定：<br />
						&nbsp;&nbsp;&nbsp;<label><input type="checkbox" name="wts[xysx2][]" value="0" />需要对女性作妇科检查；</label><br />
						&nbsp;&nbsp;&nbsp;<label><input type="checkbox" name="wts[xysx2][]" value="1" />需要对未成年人的身体进行检查；</label><br />
						&nbsp;&nbsp;&nbsp;<label><input type="checkbox" name="wts[xysx2][]" value="2" />需要对被鉴定人进行法医精神病鉴定；</label><br />
						&nbsp;&nbsp;&nbsp;<label><input type="checkbox" name="wts[xysx2][]" value="3" />需要到现场提取检材；</label><br />
						&nbsp;&nbsp;&nbsp;<label><input type="checkbox" name="wts[xysx2][]" value="4" />需要进行尸体解剖。</label><br />
						4.<label><input type="checkbox" name="wts[xysx3][]" value="0"/>需要补充或者重新提取鉴定材料的，延长<input type="text" name="wts[bctq]" value=""/>个工作日。</label><br />
						&nbsp;&nbsp;&nbsp;<label><input type="checkbox" name="wts[xysx3][]" value="1" />委托人要求鉴定人回避。被要求回避的鉴定人姓名<input type="text" name="wts[hbjdr]" value="无"/>。</label><br />
  ...
  <div id="接谈笔录">
  	<form action="<?php echo U('Jbxx/index',array('id'=>'jietanbl'));?>" method="post">
		...
		<table class="jtblcon">
			...
			<tr>
				<td id="cons">
					申请鉴定事项：<input type="text" name="jtbl[jdsx]" value="<?php echo ($a[0][name]); echo ($b[0][name]); ?>" />
				</td>
			</tr>
			<tr>
				<td id="cons">
		...
		<p></p>
		<div id="bldiv" contenteditable="true" onkeyup="bl()"></div>
